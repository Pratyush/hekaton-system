...
use ark_ec::{AffineRepr, models::short_weierstrass::SWCurveConfig as Config, short_weierstrass::Affine};
use ark_ff::Field;
use ark_std::{One, Zero};

pub struct BatchAdder<P: Config> {
    inverse_state: P::BaseField,
    inverses: Vec<P::BaseField>,
}

impl<P: Config> BatchAdder<P> {
...
mod batch_add_tests {
    ...
    fn test_phase_one_zero_or_neg() {
        ...
        let p_affine = G1Affine::rand(&mut rng);
    ...
    fn test_phase_two_p_add_neg() {
        ...
        let mut rng = ark_std::test_rng();
        let mut acc = G1Affine::rand(&mut rng);
        let mut p = acc;
        p.y = -p.y;
    ...
    fn test_batch_add_indexed_single_bucket() {
        ...
        let mut buckets: Vec<G1Affine> = vec![G1Affine::rand(&mut rng)];
        let points: Vec<G1Affine> = vec![G1Affine::rand(&mut rng)];
