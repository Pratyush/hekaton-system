...
pub fn decompose(
    ...
    (
        G1ScalarField::from(BigInt([quotient0 as u64, quotient1 as u64, 0, 0])),
        G1ScalarField::from(BigInt([r0, r1, 0, 0])),
        is_neg_scalar,
        is_neg_remainder,
    )
}
...
fn glv_preprocess_scalar(
    ...
    let mut s = [
        scalar.into_bigint().as_ref()[0],
        scalar.into_bigint().as_ref()[1],
        scalar.into_bigint().as_ref()[2],
        scalar.into_bigint().as_ref()[3],
    ];
